...
const SignUpScreen = () => {
  const {
    ...
    defaultValues: { signUpMethod: "mobile", region: "+91" },
  });
  const navigate = useNavigate();
  const { signUpMethod, showConfirmPassword, showPassword, isPending } =
    watch();
  ...
  const onsubmit = async (data: SignUpFormValues) => {
    ...
    const { email, mobileNumber, password, rePassword, region } = data;
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/user/signup`,
        {
          password,
          rePassword,
          ...(signUpMethod === "email" ? { email } : { mobileNumber, region }),
        }
      );
  ...
  return (
    <div className="relative min-h-screen w-full bg-[url('/images/login-bg.jpg')] bg-cover bg-center ">
      ...
      <div className="hidden absolute md:flex  top-6 left-6 z-20 items-center gap-4 text-white">
        <button
          onClick={() => navigate(-1)}
          className="hover:bg-white/10 py-2 px-4 cursor-pointer rounded-full transition-colors text-white"
        >
          {"<"}{" "}
        </button>
        <LogoWhite />
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-4 py-12">
        <div className="w-full max-w-[500px] rounded-2xl border border-white/20 bg-[#2033F4]/10 p-6 md:p-10 shadow-2xl backdrop-blur-md">
          <h2 className="text-center text-2xl font-bold text-white mb-8">
            Sign Up Using Number or email
          </h2>

          {/* Toggle Switch (Mobile vs Email) */}
          <div className="bg-[#242b4d]/50 p-1 rounded-lg flex mb-6 border border-white/10">
            <button
              onClick={() => setValue("signUpMethod", "mobile")}
            ...
            <button
              onClick={() => setValue("signUpMethod", "email")}
              className={`flex-1 py-2 rounded-md text-sm font-medium transition-all duration-300 ${
                signUpMethod === "email"
                  ? "bg-[#1D7ED8] text-white shadow-md"
                  : "text-gray-400 hover:text-white"
          ...
          <form onSubmit={handleSubmit(onsubmit)} className="space-y-5">
            {/* Conditional Input (Email or Mobile) */}
            <div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <div
                  className="absolute inset-y-0 right-0 pr-3 flex items-center cursor-pointer text-gray-400 hover:text-white"
                  onClick={() =>
                    setValue("showConfirmPassword", !showConfirmPassword)
                  }
                >
                  {showConfirmPassword ? (
                    <img src="/icons/eye-close.svg" alt="eye-icon" />
